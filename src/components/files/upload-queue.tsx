...
export function UploadQueue({
  ...
  const progressPercent = items.length > 0 ? (done / items.length) * 100 : 0;
    ...
    <div className="fixed bottom-4 left-1/2 z-50 w-full max-w-sm -translate-x-1/2 overflow-hidden rounded-2xl border border-border/50 bg-card/95 shadow-float backdrop-blur-xl animate-in slide-up-fade duration-300 sm:left-auto sm:right-6 sm:translate-x-0">
        ...
        <div className="flex items-center gap-2.5">
          {inProgress ? (
            <>
              ...
              <span className="text-sm font-semibold">Uploading {done}/{items.length}</span>
            </>
          ) : failed > 0 ? (
            <>
              <AlertCircle className="h-4 w-4 text-destructive" />
              <span className="text-sm font-semibold">{done} done, {failed} failed</span>
            </>
          ) : (
            <>
      ...
      <div className="h-[2px] bg-muted/20">
        <div
          ...
          style={{ width: `${inProgress ? progressPercent : 100}%` }}
        />
      </div>
        ...
        {items.map((item) => (
          ...
          >
            ...
            <div className="min-w-0 flex-1">
              ...
              {item.status === 'error' ? (
                <p className="mt-0.5 truncate text-[10px] text-destructive/80">{item.error ?? 'Failed'}</p>
              ) : (
            ...
            {item.status === 'error' && (
              <button
                ...
                className="shrink-0 rounded-lg p-1.5 text-muted-foreground transition-all hover:bg-accent hover:text-foreground active:scale-95"
                title="Retry"
              >
