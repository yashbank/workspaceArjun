...
| 25 | 2026-09-22 | sonnet | Half A complete — awaiting migrate | Phase 25 payroll rules (MIS-216 family) | phase-reports/phase-25.md | D24, D26, D27, D28, D33, F-33 | SCHEMA GATE Half A only. New: `MisEmployee.wageTypeCode/payType/sundayPaid`, `MisWageType.otRatePerHour/multiplierBasis/hraAmount/allowanceAmount/bonusAmount`, `MisEmployeePayComponent` (per-employee payslip row toggles), `MisExtraPayDay` + 2 scope join tables (D28), `MisPayrollPeriod` + `MisPayrollSnapshotLine` (D27's missing "closed month stays closed" half, backs W9). SQL in `prisma/migrations-pending/20260926000000_mis_payroll_rework/`. New D33 records 5 schema-shape choices the brief left open. tsc silent (no TS touches the new models yet); no vitest/build run. Next: human runs the gate commands (in the report), then Half B (new session) builds 25.1–25.5, closes the loop on D27, and rebuilds the payroll screen to W9 (closes F-33). |
| 25 | 2026-09-26 | sonnet | DONE | Phase 25 payroll rules (MIS-216 family) | phase-reports/phase-25.md | D24, D26, D27, D28, D33, F-33 | Half B. Migration applied. Built 25.1–25.5 end to end: per-employee payslip-row toggles, D26 OT-per-hour (falls back to the old multiplier for anyone with no code), D28 Sunday pay for MONTHLY employees and extra-pay days (propose→Owner-approve, folded into payroll, additive with OT), and D27's missing half — `closePayrollPeriod` freezes a month into `MisPayrollSnapshotLine`; `calculateMonthlyPayroll` reads it back once CLOSED, never recomputing. `/mis/payroll` rebuilt to W9 (closes F-33) — counts only, no rates/amounts, for every role including Owner. Payslip is now the six-row breakdown. Found and fixed one real leak while rebuilding: the old payroll page sent every money field to the browser as an unused prop; now stripped server-side (`payroll-screen-no-money.test.ts`). tsc silent; vitest 187 files / 4149 passed + 9 expected-fail (was 172/3868/10 — one F-05 case flipped from `.fails` to passing); build passes, every route. Disclosed test edits: `wage-screens.test.tsx` (payroll page now `moneyFreeForAll`, +1 importer registry row), `employee-slice.test.ts` (2 reviewed non-money name allow-lists), `audit-payloads.test.ts` (+1 safe key), `approvals-access.test.ts` (seeded the new gate properly). No schema change. Added skill `mis-fast-phase` (batch a phase in one pass, tsc after each unit, vitest/build once at the end). Left honest: DEPARTMENTS-scope picker (SAO lacks masters.read — BE supports it, FE doesn't offer it), automatic back-dated-write detection for `correctionsAfterClose` (manual for now). |
| 19 | 2026-09-26 | sonnet | DONE | Phase 19 QA attendance/kiosk/payroll (MIS-234, 239, 242, 247, 254, 259) | phase-reports/phase-19.md | D20, D22, D26, D27, D28 | QA only, no application code touched. Most of this ticket set was already built by Phases 13/14F/25 — this phase read that, then named the two real gaps: `attendance-payroll-rules.test.ts` (new, 4 tests) gives ABSENT and HALF_DAY their own named test (MIS-247, previously proven only inside one combined test) and proves a night-shift-filed attendance row prices identically to a day-shift one (MIS-234's missing half; punch-to-day filing was already proven in Phase 13). `docs/qa/attendance-walkthrough.md` (new) is MIS-259's reconciliation method plus one worked example on the existing fixture — explicitly NOT a real reconciliation, since no real factory month was available this session. F-08's second half marked done (Phase 25 closed it). MIS-239/MIS-242 (real kiosk device) still cannot run, no physical tablet, unchanged from Phase 12/13. tsc silent; vitest 188 files / 4153 passed + 9 expected-fail (+4 tests, was 187/4149). No build (tests/docs only phase). phase-18.md report is missing (Session B's own track) — noted, not blocking. |
| 20 | 2026-09-26 | sonnet | DONE | Phase 20 QA store/PO/scheduling/platform (MIS-279, 98, 264, 266, 81, 84, 87, 89, 93) | phase-reports/phase-20.md | D1, D4, D22 | QA only, no application code touched. MIS-279 was the real gap: no po.test.ts existed before this phase despite the buffer-stock PO feature already being built. New po-purpose.test.ts (4) and po-buffer-stock.test.ts (8, new fake db) cover all six MIS-279 cases plus the partial-receipt arithmetic (order 100, receive 40 then 35, PARTIAL at 75, COMPLETE at 100). MIS-98/264/266 already thoroughly covered by worker-allocation.test.ts (25) and machines-board.test.ts (8) - cited, not duplicated. MIS-81: new tap-targets-kit.test.tsx (5) pins Input/NumberInput/DateInput/TimeInput/Select at 48px/16px, alongside 24G's existing Button test. Timezone sweep (D22) confirmed every site Phase 13 named is still on the server's clock - consolidated into new F-34 (exact file:line catalog), not fixed (QA phase). Placeholder-wiring sweep (Phase 8's stamp) found no new drift in the six role homes or nav badges. MIS-84/89/93 need hardware/a live deploy - logged with new docs/qa/store-walkthrough.md and docs/qa/release-smoke.md (checklists, not executed rehearsals). tsc silent; vitest 191 files / 4170 passed + 9 expected-fail (was 188/4153, +17 tests). No build (tests/docs only). |
